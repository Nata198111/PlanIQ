...
export function renderAnalytics() {
  ...
  let hm='';for(let i=0;i<84;i++){hm+=`<div class="w-3 h-3 rounded-sm" style="background:${rH()}"></div>`;}
...
export function initAnalytics() {
    ...
    const calculateCoreStats = (completedTasks) => {
        statCompleted.innerHTML = completedTasks.length;
        let totalHrs = 0;
        let countWithTime = 0;
        ...
        if (countWithTime > 0) {
            const avg = (totalHrs / countWithTime).toFixed(1);
            statAvgTime.innerHTML = `${avg} <span class="text-xl font-medium opacity-50">год</span>`;
        } else {
    ...
    const renderProdChart = () => {
        ...
        if (as.prodPeriod === 'Тиждень') {
            for (let i = 6; i >= 0; i--) {
                ...
                counts.push(tasks.filter(t => t.date === str).length);
            }
            ...
            for(let i=3; i>=0; i--) {
                ...
                let weekCount = 0;
                for(let d=0; d<7; d++) {
                    const day = new Date(tY, tM, tD - (i*7 + d));
                    weekCount += tasks.filter(t => t.date === getDayStr(day)).length;
                }
        ...
        prodChart.innerHTML = labels.map((l, i) => {
            ...
            const height = Math.max(5, h); // Minimum 5% to show an empty track marker
            return `
              <div class="flex flex-col items-center gap-3 flex-1 group h-full justify-end animate-fade-in" style="animation-delay: ${i*0.05}s">
                <div class="w-full bg-gradient-to-t from-[#c4c0ff]/20 to-[#4ddada]/60 rounded-t-lg transition-all duration-300 ease-out 
                            group-hover:to-[#4ddada] group-hover:shadow-[0_0_15px_rgba(77,218,218,0.3)] border-t border-[#4ddada]/50 
                            ${selected ? ' shadow-[0_0_20px_rgba(108,99,255,0.2)]' : ''} relative" 
                     style="height:${height}%">
    ...
    const renderCatChart = () => {
        ...
        const anchor = as.anchorDate.getTime();
        
        if (as.catPeriod !== 'all') {
            const days = parseInt(as.catPeriod);
            const limitMs = anchor - (days * 86400000);
        ...
        sorted.forEach((item, index) => {
            if (index < 4) topCats.push({ id: item[0], count: item[1] });
            else othersCount += item[1];
        });
        ...
        catChart.innerHTML = `
          <div class="relative w-48 h-48 mb-8 group animate-fade-in">
