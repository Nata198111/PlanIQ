...
export function renderTopbar(title, options = {}) {
  ...
  return `
    <header class="h-16 w-full sticky top-0 z-40 bg-[#0F0F1A]/80 backdrop-blur-xl flex justify-between items-center px-4 md:px-8 shadow-[0_1px_10px_rgba(108,99,255,0.05)]">
      ...
      <div class="flex items-center gap-3 md:gap-4 flex-shrink-0">
        ...
        <!-- Кнопка сповіщень з badge -->
        <a href="#/notifications" class="relative w-9 h-9 md:w-10 md:h-10 rounded-xl bg-[#1b1a26] hover:bg-[#292935] border border-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-all" id="topbar-notif-btn" title="Сповіщення">
          <span class="material-symbols-outlined text-[20px]">notifications</span>
          <!-- Badge — оновлюється через updateBadge() -->
          <span id="topbar-notif-badge" class="hidden absolute -top-1 -right-1 bg-[#ffb2bc] text-[#1b1a26] text-[9px] font-black rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-0.5 shadow-lg"></span>
        </a>
...
export function updateTopbarBadge(count) {
