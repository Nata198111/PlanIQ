...
export function renderSidebar(activePage, isCollapsed = false) {
  ...
  const textClass = isCollapsed ? 'lg:w-0 lg:opacity-0 lg:hidden' : 'whitespace-nowrap transition-all duration-300';
  ...
  <aside id="sidebar" class="w-[280px] ${isCollapsed ? 'lg:w-[80px]' : 'lg:w-[240px]'} h-screen fixed left-0 top-0 bg-[#0F0F1A] border-r border-white/5 flex flex-col py-6 z-50 shadow-2xl shadow-indigo-500/5 transition-all duration-300 -translate-x-full lg:translate-x-0">
    ...
    <div class="mt-auto px-2 space-y-1">
      ...
      <a href="#/notifications" class="${activePage === 'notifications' ? 'bg-[#6C63FF]/10 text-[#6C63FF] border-l-2 border-[#6C63FF] font-semibold' : 'text-slate-500 hover:bg-slate-800/50'} ${isCollapsed ? 'lg:px-0 lg:border-l-0 lg:rounded-lg' : 'px-4'} py-3 rounded-lg lg:rounded-r-lg mx-2 lg:mx-0 transition-all flex items-center justify-center lg:justify-start gap-3 group" id="notifications-btn" title="Сповіщення">
        <span class="material-symbols-outlined group-hover:text-white transition-colors" ${activePage === 'notifications' ? 'style="font-variation-settings: \'FILL\' 1;"' : ''}>notifications</span>
