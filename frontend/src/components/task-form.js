...
export function renderTaskForm(task = null) {
  ...
  return `
    <style>
      ...
      /* Hide native calendar/time browser icons */
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="relative active-selector-cont" id="date-selector-cont">
          ...
          <div class="relative group cursor-pointer" id="date-trigger">
            ...
            <input type="text" id="form-date" value="${task?.date || new Date().toISOString().slice(0, 10)}" readonly
...
export function initTaskForm(container, task = null, submitBtn = null) {
  ...
  let state = {
    ...
    selectedDate: task?.date || new Date().toISOString().slice(0, 10)
  };
  ...
  const updateCatSelection = () => {
    catGroup.querySelectorAll('.form-chip').forEach(btn => {
      ...
      btn.className = `form-chip px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border ${active ? 'active' : 'bg-[#1b1a26] border-transparent text-slate-500 hover:text-slate-300'}`;
    });
    validate();
  };

  catGroup.addEventListener('click', (e) => {
    if (e.target.classList.contains('form-chip')) {
      state.category = e.target.dataset.val;
      updateCatSelection();
    }
  });
  ...
  const catInputField = form.querySelector('#new-cat-input');
  ...
  catSaveBtn.onclick = () => {
    const name = catInputField.value.trim();
    if (name) {
      const newId = taskStore.addCategory(name);
  ...
  drpMenu.querySelectorAll('.dropdown-item').forEach(item => {
    item.onclick = () => {
      ...
      state.status = val;
      drpLabel.textContent = val;
      drpMenu.classList.add('hidden');
      drpMenu.querySelectorAll('.dropdown-item').forEach(i => {
        i.classList.toggle('text-[#c4c0ff]', i.dataset.val === val);
        i.classList.toggle('font-bold', i.dataset.val === val);
      });
      validate();
    };
  });
  ...
  function initTimePicker() {
      ...
      timeMenu.innerHTML = times.map(t => `<button type="button" class="w-full text-left px-4 py-2 text-xs text-slate-300 hover:bg-white/5 hover:text-white transition-all font-mono" data-val="${t}">${t}</button>`).join('');
      timeMenu.querySelectorAll('button').forEach(btn => {
          btn.onclick = () => {
              timeInput.value = btn.dataset.val;
  ...
  function renderCalendar(year, month) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const isActive = state.selectedDate === curDateStr;
        html += `<button type="button" class="h-7 w-7 rounded-lg text-[10px] flex items-center justify-center transition-all 
            ${isActive ? 'bg-[#c4c0ff] text-[#2000a4] font-bold' : 'text-slate-400 hover:bg-white/5 hover:text-white'}" 
            data-date="${curDateStr}">${d}</button>`;
    }
    html += `</div>`;
    
    dateMenu.innerHTML = html;
    
    dateMenu.querySelector('#cal-prev-mo').onclick = (e) => { e.stopPropagation(); renderCalendar(month === 0 ? year - 1 : year, month === 0 ? 11 : month - 1); };
    dateMenu.querySelector('#cal-next-mo').onclick = (e) => { e.stopPropagation(); renderCalendar(month === 11 ? year + 1 : year, month === 11 ? 0 : month + 1); };
